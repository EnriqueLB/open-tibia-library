...
export class WebsiteImageGeneratorBase {
    ...
    init() {
        ...
        this.generateImagesButton.onclick = function () {
            if (!self.spriteManager || !self.datManager || !self.otbManager) {
                self.progressText(self.getMissingFilesMessage());
                return;
            }
    ...
    protected getMissingFilesMessage() {
        if (this.otbRequired) {
            return 'Cannot generate images. First .spr, .dat and .otb files must be loaded.';
        }

        return 'Cannot generate images. First .spr and .dat files must be loaded.';
    }

    protected getPostDatLoadingProgressText() {
        if (this.otbRequired) {
            return 'Loading OTB file';
        }

        return 'Finalizing data load';
    }
    ...
    loadDat() {
        if (this.datPicker.files.length > 0) {
            ...
            reader.onload = function (event: any) {
                ...
                if (datLoaded) {
                    self.progressText(self.getPostDatLoadingProgressText());
                    setTimeout(function () {
                        self.loadOtb()
                    }, 10);
                } else {
    ...
    loadOtb() {
        if (this.otbPicker && this.otbPicker.files.length > 0) {
