...
class Outfitter {
    ...
    protected static $outfitFolderIndex = null;
	...
	public static function loadData($outfitId, $isMount = false)
	{
        ...
        $outfitFolderPath = self::getOutfitFolderPath($outfitId);
		if($outfitFolderPath && file_exists($outfitFolderPath . '/outfit.data.txt'))
		{
			if($isMount)
			{
				$tmp = unserialize(file_get_contents($outfitFolderPath . '/outfit.data.txt'));
				self::$data['files'] = array_merge(self::$data['files'], $tmp['files']);
				self::$data['mountFramesNumber'] = $tmp['framesNumber'];
                self::$data['mountFolderPath'] = $outfitFolderPath;
                self::$data['mountFileMap'] = isset($tmp['fileMap']) ? $tmp['fileMap'] : [];
			}
			else
			{
				self::$data = unserialize(file_get_contents($outfitFolderPath . '/outfit.data.txt'));
				self::$data['mountFramesNumber'] = 1;
                self::$data['folderPath'] = $outfitFolderPath;
                self::$data['fileMap'] = isset(self::$data['fileMap']) ? self::$data['fileMap'] : [];
			}
			return true;
		}
		return false;
    ...
    protected static function getOutfitFolderIndex()
    {
        if (self::$outfitFolderIndex !== null) {
            return self::$outfitFolderIndex;
        }

        self::$outfitFolderIndex = [];
        $cacheIndexPath = self::$outfitPath . 'outfit.cache.index.json';
        if (file_exists($cacheIndexPath)) {
            $decodedIndex = json_decode(file_get_contents($cacheIndexPath), true);
            if (is_array($decodedIndex)) {
                self::$outfitFolderIndex = $decodedIndex;
                return self::$outfitFolderIndex;
            }
        }

        $exportIndexPath = self::$outfitPath . 'outfit.index.json';
        if (file_exists($exportIndexPath)) {
            $decodedIndex = json_decode(file_get_contents($exportIndexPath), true);
            if (is_array($decodedIndex)) {
                self::$outfitFolderIndex = $decodedIndex;
            }
        }
    ...
    protected static function getOutfitFolderPath($outfitId)
    {
        if (file_exists(self::$outfitPath . $outfitId . '/outfit.data.txt')) {
            return rtrim(self::$outfitPath . $outfitId, '/');
        }

        $folderIndex = self::getOutfitFolderIndex();
        if (!isset($folderIndex[$outfitId])) {
            return null;
        }

        $folderName = $folderIndex[$outfitId];
        if (is_array($folderName) && isset($folderName['folder'])) {
        ...
        return rtrim(self::$outfitPath . $folderName, '/');
    }
    ...
    protected static function getFolderBaseName($folderPath)
    {
        return basename(str_replace('\\', '/', rtrim($folderPath, '/')));
    }

    protected static function getOutfitFrameFilePath($folderPath, $animation, $mountState, $addon, $direction, $isTemplate = false, $fileMap = [])
    {
        $folderBaseName = self::getFolderBaseName($folderPath);
        $fileSuffix = $isTemplate ? '_template.png' : '.png';
        $logicalFileName = $animation . '_' . $mountState . '_' . $addon . '_' . $direction;

        if (isset($fileMap[$logicalFileName]) && is_string($fileMap[$logicalFileName])) {
            return $folderPath . '/' . $fileMap[$logicalFileName] . $fileSuffix;
        }

        $candidates = [
            $folderPath . '/' . $folderBaseName . '_' . $animation . '_' . $mountState . '_' . $addon . '_' . $direction . $fileSuffix,
            $folderPath . '/' . $animation . '_' . $mountState . '_' . $addon . '_' . $direction . $fileSuffix,
        ];

        if ($mountState == 1 && $addon == 1 && $direction == 1) {
            array_unshift(
                $candidates,
                $folderPath . '/' . $folderBaseName . '_' . $animation . $fileSuffix
            );
        }

        foreach ($candidates as $candidatePath) {
            if (self::file_exists($candidatePath)) {
	...
	public function outfit($outfit, $addons, $head, $body, $legs, $feet, $mount, $direction = 3, $animation = 1) {
        ...
        $outfitFolderPath = isset(self::$data['folderPath']) ? self::$data['folderPath'] : rtrim(self::$outfitPath . $outfit, '/');
        $mountFolderPath = isset(self::$data['mountFolderPath']) ? self::$data['mountFolderPath'] : rtrim(self::$outfitPath . $mountId, '/');
        $outfitFileMap = isset(self::$data['fileMap']) ? self::$data['fileMap'] : [];
        $mountFileMap = isset(self::$data['mountFileMap']) ? self::$data['mountFileMap'] : [];
        $baseOutfitFile = self::getOutfitFrameFilePath($outfitFolderPath, $animation, $mountState, 1, $direction, false, $outfitFileMap);
        if ($baseOutfitFile) {
            $image_outfit = imagecreatefrompng($baseOutfitFile);
            $baseTemplateFile = self::getOutfitFrameFilePath($outfitFolderPath, $animation, $mountState, 1, $direction, true, $outfitFileMap);
            if ($baseTemplateFile) {
                $image_template = imagecreatefrompng($baseTemplateFile);
            } else {
            ...
            if ($addons == 1 || $addons == 3) {
                $firstAddonFile = self::getOutfitFrameFilePath($outfitFolderPath, $animation, $mountState, 2, $direction, false, $outfitFileMap);
                if ($firstAddonFile) {
                    $image_first = imagecreatefrompng($firstAddonFile);
                    $this->alphaOverlay($image_outfit, $image_first, 64, 64);
                    imagedestroy($image_first);
                    $firstAddonTemplateFile = self::getOutfitFrameFilePath($outfitFolderPath, $animation, $mountState, 2, $direction, true, $outfitFileMap);
                    if ($firstAddonTemplateFile) {
                        $image_first_template = imagecreatefrompng($firstAddonTemplateFile);
            ...
            if ($addons == 2 || $addons == 3) {
                $secondAddonFile = self::getOutfitFrameFilePath($outfitFolderPath, $animation, $mountState, 3, $direction, false, $outfitFileMap);
                if ($secondAddonFile) {
                    $image_second = imagecreatefrompng($secondAddonFile);
                    $this->alphaOverlay($image_outfit, $image_second, 64, 64);
                    imagedestroy($image_second);
                    $secondAddonTemplateFile = self::getOutfitFrameFilePath($outfitFolderPath, $animation, $mountState, 3, $direction, true, $outfitFileMap);
                    if ($secondAddonTemplateFile) {
                        $image_second_template = imagecreatefrompng($secondAddonTemplateFile);
        ...
        $mountFile = self::getOutfitFrameFilePath($mountFolderPath, $mountAnimationFrame, 1, 1, $direction, false, $mountFileMap);
		if ($mountState == 2 && $mountFile) {
			$mount = imagecreatefrompng($mountFile);
